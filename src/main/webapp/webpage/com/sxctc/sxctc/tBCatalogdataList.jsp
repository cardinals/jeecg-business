...
<div class="easyui-layout" fit="true">
	<div region="center" style="padding:0px;border:0px">
		<table id="tBCatalogdataList"></table>  
	</div>
	<div id = "tBCatalogdataListToolbar">
		<div class="easyui-panel toolbar-search" style="display:none" data-options="doSize:false">
			<form id="tBCatalogdataForm" onkeydown="if(event.keyCode==13){doSearch();return false;}">
		...
		<div class="toolbar-btn">
			...
			<button type="button" class="tool-btn tool-btn-default tool-btn-xs" onclick="update('编辑','tBCatalogdataController.do?goUpdate','tBCatalogdataList',768,500)">
				<i class="fa fa-edit"></i>
				<span>编辑</span>
			</button>
			
			<button type="button" class="tool-btn tool-btn-default tool-btn-xs" onclick="deleteALLSelect('批量删除','tBCatalogdataController.do?doBatchDel','tBCatalogdataList',null,null)">
				<i class="fa fa-trash"></i>
				<span>批量删除</span>
			</button>
			
			<button type="button" class="tool-btn tool-btn-default tool-btn-xs" onclick="openuploadwin('Excel导入', 'tBCatalogdataController.do?upload', 'tBCatalogdataList')">
				<i class="fa fa-download"></i>
				<span>导入</span>
			</button>
			
			<button type="button" class="tool-btn tool-btn-default tool-btn-xs" onclick="JeecgExcelExport('tBCatalogdataController.do?exportXls','tBCatalogdataList')">
				<i class="fa fa-upload"></i>
				<span>导出</span>
			</button>
			
			<button type="button" class="tool-btn tool-btn-default tool-btn-xs" onclick="JeecgExcelExport('tBCatalogdataController.do?exportXlsByT','tBCatalogdataList')">
				<i class="fa fa-upload"></i>
				<span>模版下载</span>
			</button>
		
			<button type="button" class="tool-btn tool-btn-default tool-btn-xs" onclick="$('.toolbar-search').slideToggle(function(){$('#tBCatalogdataList').datagrid('resize');});">
...
<script>
var tBCatalogdataListdictsData = {};
$(function(){
	var promiseArr = [];
	
    Promise.all(promiseArr).then(function(results) {
    	initDatagrid();
		$('#tBCatalogdataList').datagrid('getPager').pagination({
	        beforePageText: '',
	        afterPageText: '/{pages}',
...
function initDatagrid(){
	var actionUrl = "tBCatalogdataController.do?datagrid&field=id,name,danwei,fartherid,num,beizhu,";
 	$('#tBCatalogdataList').datagrid({
		...
		rownumbers:true,
		showFooter:true,
		toolbar: '#tBCatalogdataListToolbar',
		frozenColumns:[[]],
		columns:[[
			...
			,{
	            ...
	            formatter: function(value, rec, index) {
	                ...
	                var href = '';
	                href += "<a href='#'   class='ace_button'  onclick=delObj('tBCatalogdataController.do?doDel&id=" + rec.id + "','tBCatalogdataList')>  <i class=' fa fa-trash-o'></i> ";
	                href += "删除</a>&nbsp;";
	                return href;
	            }
	        }
		...
		onLoadSuccess: function(data) {
            $("#tBCatalogdataList").datagrid("clearSelections");
            if (!false) {
                if (data.total && data.rows.length == 0) {
                    var grid = $('#tBCatalogdataList');
...
function reloadTable() {
	 $('#tBCatalogdataList').datagrid('reload');
}
//easyui-datagrid搜索
function doSearch(){
...
function resetSearch(){
    ...
    $('#tBCatalogdataForm').find("input[type='checkbox']").each(function() {
