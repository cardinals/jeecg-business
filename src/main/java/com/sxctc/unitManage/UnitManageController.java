...
//@Scope("prototype")
@Controller
@RequestMapping("/unitManageController")
public class UnitManageController extends BaseController {
	...
	public ModelAndView typeGroupList(HttpServletRequest request) {
		return new ModelAndView("com/sxctc/unitManage/typeGroupList");
	}
			...
			for (String string : set) {
				...
				if (map1.containsKey(string)) {
					if ("createDate".equals(string)&&StringUtil.isNotEmpty(map1.get(string))){
						Date date=new Date((String) map1.get(string));
				...
				if (map2.containsKey(string)) {
					if ("createDate".equals(string)&&StringUtil.isNotEmpty(map2.get(string))){
						Date date=new Date((String) map2.get(string));
						SimpleDateFormat simpledateformat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
